...
if($_SERVER['REQUEST_METHOD']=='GET'){
    ...
    do{
        if(empty($id)||empty($addr)||empty($name)||empty($email)||empty($phone)||empty($password)){
            $errorMessage="All the fields are required";
            break;
        }

        $sql="UPDATE `customer` SET `cid`='$id',`caddr`='$addr',`cname`='$name',`cmail`='$email',`cphone`='$phone',`password`='$password' WHERE cid=$id";
        $result=$conn->query($sql);
