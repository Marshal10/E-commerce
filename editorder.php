...
if($_SERVER['REQUEST_METHOD']=='GET'){
    ...
    do{
        if(empty($id)||empty($name)||empty($addr)||empty($email)||empty($phone)||empty($product)||empty($payment)){
            $errorMessage="All the fields are required";
            break;
        }

        $sql="UPDATE `orders` SET `oid`='$id',`username`='$name',`addr`='$addr',`email`='$email',`phone`='$phone',`product`='$product',`payment`='$payment' WHERE oid=$id";
        $result=$conn->query($sql);
        ...
        $successMessage="Order Updated  Succesfully";
        header("location:orders.php");
        exit;
